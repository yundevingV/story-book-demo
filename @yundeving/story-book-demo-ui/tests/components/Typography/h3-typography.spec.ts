...
const storyId = "typography--h-3";

test.describe("Typography H3 구조/정렬", () => {
  ...
  test("variant=h3 는 role=heading level=3 로 렌더링", async ({ page }) => {
    await page.goto(`/iframe.html?id=${storyId}`);
    const story = page.locator("#storybook-root");
    const heading = story.getByRole("heading", { level: 3 });
  ...
  test("className 주입 테스트", async ({ page }) => {
    ...
    const story = page.locator("#storybook-root");
    const hCenter = story.locator("h3").first();
