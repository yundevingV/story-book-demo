...
const storyId = "typography--h-1";

test.describe("Typography H1 구조/정렬", () => {
  ...
  test("variant=h1 는 role=heading level=1 로 렌더링", async ({ page }) => {
    await page.goto(`/iframe.html?id=${storyId}`);
    const story = page.locator("#storybook-root");
    const heading = story.getByRole("heading", { level: 1 });
  ...
  test("className 주입 테스트", async ({ page }) => {
    ...
    const story = page.locator("#storybook-root");
    const hCenter = story.locator("h1").first();
