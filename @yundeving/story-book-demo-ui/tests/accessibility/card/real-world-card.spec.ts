...
test.describe("Real Word Card 접근성 테스트", () => {
  // before start test
  test.beforeEach(async ({ page }) => {
    await initPage(page);

    // Card 스토리가 존재하는지 확인하고 적절한 스토리로 이동
    await page.goto("/iframe.html?id=card--real-word-exaple&viewMode=story");
  ...
  test("Real Word Card 기본 접근성 위반사항 없어야 함", async ({ page }) => {
  ...
  test("Real Word Card 색상 대비 테스트 (라이트 모드)", async ({ page }) => {
    // 라이트 모드 확인
    await page.evaluate(() => {
...
test.describe("Real Word Card 다크 모드 테스트", () => {
  ...
  test("Real Word Card 색상 대비 테스트 (다크 모드)", async ({ page }) => {
