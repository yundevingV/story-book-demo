...
test.describe("With Action Card 접근성 테스트", () => {
  ...
  test("With Action Card 기본 접근성 위반사항 없어야 함", async ({ page }) => {
  ...
  test("With Action Card 색상 대비 테스트 (라이트 모드)", async ({ page }) => {
    // 라이트 모드 확인
    await page.evaluate(() => {
  ...
  test("With Action Card 키보드 접근성 테스트", async ({ page }) => {
    // Card가 포커스 가능한지 확인
    const card = page
...
test.describe("With Action Card 다크 모드 테스트", () => {
  ...
  test("With Action Card 색상 대비 테스트 (다크 모드)", async ({ page }) => {
