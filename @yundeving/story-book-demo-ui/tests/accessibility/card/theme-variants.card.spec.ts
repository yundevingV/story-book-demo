...
test.describe("Theme Variants Card 접근성 테스트", () => {
  // before start test
  ...
  test("Theme Variants Card 기본 접근성 위반사항 없어야 함", async ({
    page,
  }) => {
  ...
  test("Theme Variants Card 색상 대비 테스트 (라이트 모드)", async ({
    page,
  }) => {
    // 라이트 모드 확인
    await page.evaluate(() => {
...
test.describe("Theme Variants Card 다크 모드 테스트", () => {
  ...
  test("Theme Variants Card 색상 대비 테스트 (다크 모드)", async ({ page }) => {
